...
const { evaluateThresholds } = require('./alerts');
...
async function checkSite(site) {
  ...
  const triggered = evaluateThresholds(conditions);
  ...
  for (const alert of triggered) {
    ...
    console.log(`  [${site.name}] ${alert.label} — ${alert.actual}${alert.unit} exceeds threshold ${alert.threshold}${alert.unit}`);

    // Generate email
    const subject = `[WEATHER ALERT] ${alert.label} — ${site.name}`;
